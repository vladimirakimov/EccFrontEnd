...
import { TYPES, ACTIONS as LIST_ACTIONS } from './operationList.actions'
...
const getOriginalRows = (state, changes) => getOperations(state).filter(operation => changes[operation.id])
const getOriginalRow = (state, key) => getOperations(state).filter(operation => operation.id === key)[0]
const getDeletingRowIds = state => state.pages.operations.deletingRowIds
...
export default operationListSaga
